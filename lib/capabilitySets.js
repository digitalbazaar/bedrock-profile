...
bedrock.events.on('bedrock-mongodb.ready', async () => {
  ...
  await promisify(database.createIndexes)([{
    // cover capability set queries by profileAgent ID
...
// get all capability sets for a profile agent
exports.getAll = async ({profileAgentId} = {}) => {
  assert.optionalString('profileAgentId', profileAgentId);
  ...
  const records = await collection.find(query,
    {_id: 0, capabilitySet: 1, meta: 1}).toArray();
  return records;
};
