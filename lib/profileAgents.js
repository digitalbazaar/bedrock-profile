...
export async function getAll({accountId, includeSecrets = false} = {}) {
  ...
  if(!includeSecrets) {
    records = await removeSecretsFromRecords({records});
  }
...
export async function update({profileAgent} = {}) {
  ...
  const result = await collection.findOneAndUpdate(
    query, {$set}, {returnNewDocument: true});
...
async function _refreshProfileAgentZcaps({profileAgentRecords}) {
  ...
  for(let record of records) {
    ...
    if(!zcaps?.profileCapabilityInvocationKey) {
      // cannot refresh without this zcap
      continue;
    }
    ...
    if(zcapRefreshed) {
      ...
      record = await update({profileAgent: record.profileAgent});
    }
  }
  if(zcapRefreshed) {
    return records;
