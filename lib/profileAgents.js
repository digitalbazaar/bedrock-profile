      ...
      if(zcapName !== 'profileCapabilityInvocationKey') {
        ...
        if(expiresTimestamp - now <= zcapRefreshThreshold) {
          if(zcapName === 'userDocument') {
            ...
            const edvConfig = await _getEdvConfig({edvId, profileSigner});
            const docId = zcaps.userDocument.invocationTarget.split('/').pop();
            const res = await _getEdvDocument({
              docId, edvConfig, edvId, profileSigner
            });
...
async function _getEdvConfig({edvId, profileSigner}) {
...
async function _getEdvDocument({docId, edvConfig, edvId, profileSigner}) {
