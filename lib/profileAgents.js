...
/**
 * Creates a ProfileAgent and associates it to an account if provided
 *
 * @param {Object} account the account creating the ProfileAgent.
 *
 ...
 * @return {Promise<ProfileAgent>} resolves to a ProfileAgent.
 */
exports.getByProfile = async ({profile}) => {
  assert.string(profile, 'profile');

  const query = {'profileAgent.profile': profile};
  const collection = utils.getCollection('profile-profileAgent');
  const record = await collection.findOne(query, {_id: 0});
  if(!record) {
    const details = {
