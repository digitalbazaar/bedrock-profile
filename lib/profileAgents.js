 ...
 *
 * @returns {Promise<object>} Resolves to a update result with the updated
 *   record in `value`.
 */
...
async function _refreshZcaps({record}) {
  ...
  while(true) {
    ...
    console.log('existing zcaps', existingZcaps);
    ...
    const newRecord = klona(record);
    console.log('THE NEW zcaps', zcaps);
    newRecord.profileAgent.zcaps = {...newRecord.profileAgent.zcaps, ...zcaps};
    console.log('THE PROPOSED NEW RECORD', newRecord.profileAgent);
    ...
    try {
      ...
      const {value} = await update({profileAgent: newRecord.profileAgent});
      console.log('THE UPDATED RECORD', value.profileAgent);
      return value;
    } catch(e) {
      if(e.name !== 'InvalidStateError') {
        throw e;
...
async function _generateProfileAgentZcaps({profileSigner, now, zcaps}) {
  ...
  for(const [zcapName, zcap] of Object.entries(zcaps)) {
    if(zcapName === 'profileCapabilityInvocationKey') {
      // profile capability invocation key zcap cannot be refreshed
      continue;
    }
    refreshMap.set(zcapName, _refreshZcap({profileSigner, now, zcap}));
  }
...
async function _refreshUserEdvDocZcaps({
  ...
  while(true) {
    ...
    // FIXME: remove me
    console.log('*****existing profile agent user EDV doc',
    ...
    // FIXME: remove me
    console.log('*****THE NEW profile agent user EDV doc',
      JSON.stringify(doc.content, null, 2));
...
async function _refreshZcap({profileSigner, now, zcap}) {
  // FIXME: use new config value
  const expires = new Date(now + EDV_ZCAP_TTL);
  return zcaps.delegate({
    allowedActions: zcap.allowedAction,
    capability: zcap.proof.capabilityChain[0],
