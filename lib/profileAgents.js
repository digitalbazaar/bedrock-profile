...
let EDV_ZCAP_TTL;
let MAX_ZCAP_SYNC_DELTA;

bedrock.events.on('bedrock.start', async () => {
  // FIXME: rename `EDV_ZCAP_TTL` and validate all other zcap config params
  // here

  //const {zcapRefreshThreshold} = config.profile.profileAgent;
  ({edvZcapTtl: EDV_ZCAP_TTL} = config.profile.profileAgent);
  // FIXME: include in config and validate range
  // 10 minute sync difference max; this must be less than the minimum
  // auto-refresh threshold (which has a hard minimum of 15 minutes)
  MAX_ZCAP_SYNC_DELTA = 1000 * 60 * 10;
});
...
async function _refreshZcaps({record}) {
  ...
  2. Establish a baseline `now` working time to use consistently across all
    comparisons.
  3. Generate new `userDocument`, `user-edv-documents`, and `user-edv-kak`
    zcaps.
  ...
  5. If the user document has other zcaps delegated by the profile, then
    ...
    they were delegated less than `MAX_ZCAP_SYNC_DELTA` before `now`; this
    is used instead of the zcap auto-refresh time again to ensure that the
    zcaps are kept reasonably in-sync, but to allow for other processes to have
    updated the user EDV document within `MAX_ZCAP_SYNC_DELTA` w/o needing
    another EDV update.
  ...
  while(true) {
    ...
    // generate refreshed zcaps and replace them in a new copy of the record
    const {zcaps} = await _generateProfileAgentZcaps({record, now});
    let newRecord = klona(record);
    newRecord.profileAgent.zcaps = zcaps;

    // refresh all auto-refreshable zcaps in the profile agent's user EDV doc
    await _refreshUserEdvDocZcaps({profileAgentRecord: newRecord, now, zcaps});

    // update the profile agent record
    try {
      newRecord = await update({profileAgent: newRecord.profileAgent});
    } catch(e) {
      if(e.name !== 'InvalidStateError') {
        throw e;
      }
...
async function _generateProfileAgentZcaps({record, now}) {
  // FIXME: implement
}
...
async function _refreshProfileAgentUserDocZcaps({zcap, now, record} = {}) {
  ...
  const edvClient = new EdvClient({id: edvId, httpsAgent, keyResolver});
...
async function _getUserEdvDocument({profileAgentRecord, zcaps}) {
  const invocationSigner = await getSigner({profileAgentRecord});
  const capability = zcaps['user-edv-documents'] || zcaps.userDocument;
  ...
  const doc = new EdvDocument({
    id: zcaps.userDocument.invocationTarget,
    capability,
    invocationSigner,
    keyAgreementKey,
    client: edvClient
...
async function _refreshUserEdvDocZcaps({profileAgentRecord, now, zcaps}) {
  ...
  // position
  const {
    capabilityInvocationKey: capabilityInvocationKeyZcap
  ...
  const earliestDelegation = now - MAX_ZCAP_SYNC_DELTA;

  while(true) {
    // read existing profile agent user EDV document
    const edvDoc = await _getUserEdvDocument({profileAgentRecord, zcaps});
    ...
    for(const [zcapName, zcap] in Object.entries(existingZcaps)) {
      ...
      if(created < earliestDelegation) {
        refreshMap.set(
          zcapName, _refreshZcap({capabilityInvocationKeyZcap, zcap}));
      }
    }
    ...
    // set HMAC zcap
    const hmac = doc.content.zcaps['user-edv-hmac'];
    if(!hmac) {
      throw new BedrockError(
        'Profile agent capabilities cannot be refreshed; missing HMAC zcap.', {
          name: 'OperationError',
          details: {
            profileAgent: id,
            httpStatusCode: 500,
...
async function _refreshZcap({capabilityInvocationKeyZcap, zcap}) {
  // FIXME: implement
}
