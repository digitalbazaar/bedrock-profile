...
import {
  AsymmetricKey, CapabilityAgent,
  // Hmac, KeyAgreementKey,
  KmsClient
...
async function _getProfileAgentRecords({accountId, includeSecrets}) {
  ...
  // check if profile agent zcaps are close to expiration, if so refresh the
  // zcaps
  ...
  if(!includeSecrets) {
    // delete secrets from the records.
  }
  return records;
}
  ...
  for(const record of records) {
    ...
    for(const zcapName in zcaps) {
      ...
      if(zcapName !== 'profileCapabilityInvocationKey') {
        ...
        if(expiresTimestamp - now <= zcapRefreshThreshold) {
          if(zcapName === 'userDocument') {
            const {data} = record.secrets.seed;
            const seed = Buffer.from(data);
            const {capabilityAgent} = await getAgents({
              profileAgent: record.profileAgent,
              secrets: {seed}
            });
            const {
              invocationSigner: profileAgentInvocationSigner
            } = capabilityAgent.getSigner();
            const kmsClient = new KmsClient({httpsAgent});
            const {
              invocationSigner: profileInvocationSigner
            } = await AsymmetricKey.fromCapability({
              capability: profileCapabilityInvocationKeyZcap,
              invocationSigner: profileAgentInvocationSigner,
              kmsClient
            });
            console.log(JSON.stringify(zcaps, null, 2), '<><><><>zcaps');
            console.log(zcaps.userDocument);
            // read from the EDV
            const docUrl = new URL(zcaps.userDocument.invocationTarget);
            // const rootEdvUrl = `${docUrl.origin}/edvs`;
            const edvId =
              // eslint-disable-next-line max-len
              `${docUrl.protocol}//${docUrl.hostname}:${docUrl.port}${docUrl.pathname.split('/').slice(0, 3).join('/')}`;
            const edvClient = new EdvClient({
              id: edvId,
              httpsAgent
            });
            const edvConfig = await edvClient.getConfig({
              invocationSigner: profileInvocationSigner,
              // capability:
              //   `${ZCAP_ROOT_PREFIX}${encodeURIComponent(rootEdvUrl)}`,
            });
            console.log(edvConfig, 'edvConfig');
            ...
            // const res = await edvClient.get({id: docId});
