...
async function _refreshZcaps({record, now, zcapRefreshThreshold}) {
  // zcaps cannot be auto-refreshed unless the zcap invocation key can be used
  ...
  // FIXME: there are only two specific zcaps to update:
  // `userDocument` and `user-edv-kak`
  // these should be refreshed first but not stored, and then used to access
  // the EDV user document, which should then be updated ... and then the
  // `userDocument` and `user-edv-kak` zcaps should be written to the
  // record ... the code below needs to be updated to specifically do these
