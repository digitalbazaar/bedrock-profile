...
import {
  AsymmetricKey, CapabilityAgent, KeyAgreementKey, KmsClient
...
bedrock.events.on('bedrock.start', async () => {
  ({edvZcapTtl: EDV_ZCAP_TTL} = config.profile.profileAgent);
});
...
export async function update({profileAgent} = {}) {
  ...
  const result = await collection.findOneAndUpdate(
    query, {$set}, {returnDocument: 'after'});
  ...
  const {buffer} = result.value.secrets.seed;
  const record = {
    ...result,
    value: {
      ...result.value,
...
export async function getSigner({profileAgentRecord} = {}) {
  // FIXME: this duplicates `getAgents`, code cleaned up
...
async function _refreshProfileAgentZcaps({records}) {
  ...
  return Promise.all(records.map(record =>
    _refreshZcaps({record, now, zcapRefreshThreshold})));
}

async function _refreshZcaps({record, now, zcapRefreshThreshold}) {
  ...
  if(!(zcaps?.profileCapabilityInvocationKey &&
    zcaps.userDocument && zcaps['user-edv-kak'])) {
  ...
  // FIXME: remove
  {
    const userDoc = await _readUserEdvDocument({profileAgentRecord: record});
    console.log('*****NEW profile agent user doc',
      JSON.stringify(userDoc.content, null, 2));
  }

  // time at which at zcap should be refreshed if it will expire by then
  const refreshTime = now + zcapRefreshThreshold;
  if(Date.parse(zcaps.userDocument.expires) > refreshTime) {
  ...
  1. Generate new `userDocument` and `user-edv-kak` zcaps.
  2. Use the new zcaps to access the EDV user document.
  3. If the user document has other zcaps that can be refreshed, do so, but
    only using a single update to the EDV document -- and ensure that if a
    conflict arises, a retry is attempted only if the zcaps still need
    refreshing.
  4. Finally, update the profile agent record if its `userDocument` zcap hasn't
    been updated since it was last read. */

  // generate refreshed zcaps
  // FIXME: this helper function needs to be reworked to only update the
  // local zcaps here -- and to do so in parallel; output should include
  // the new zcaps and the EDV doc/client perhaps?
  // await _refreshProfileAgentUserDocZcaps({zcap, now, record});

  // update EDV document zcaps
  // FIXME: the part of this helper function that updates the user EDV doc
  // needs to be split out and called here... update all zcaps found in the
  // user EDV doc that have a delegation proof with verification method
  // matching the capability invocation key can and should be refreshed
  // FIXME: use `userDocument` to fetch EDV doc ... once it's in hand,
  // refresh the zcaps inside, creating them if they aren't present, and
  // use them to update the user doc (`user-edv-documents` has write permission
  // and `user-edv-hmac` will be created/present, where as `userDocment` has
  // only `read` permission)
  // await _refreshProfileAgentUserDocZcaps({zcap, now, record});

  // update profile agent record if `userDocument` zcap hasn't changed
  // FIXME: perform specialized update to profile agent record based on
  // whether `userDocument` zcap is the same as before (check its `id` in the
  // query)

  // FIXME: old -- needs to be reworked
  let zcapRefreshed = false;
  for(const zcapName in zcaps) {
    // cannot refresh this zcap
    if(zcapName === 'profileCapabilityInvocationKey') {
      continue;
    }
    // refresh zcap if it expires before/at refresh time
    const zcap = zcaps[zcapName];
    if(Date.parse(zcap.expires) <= refreshTime) {
      if(zcapName === 'userDocument') {
        // other zcaps to refresh can be found in the EDV user doc
        await _refreshProfileAgentUserDocZcaps({zcap, now, record});
      }
      zcap.expires = timestampToDateString(now + EDV_ZCAP_TTL);
      zcapRefreshed = true;
    }
  }
  ...
  if(zcapRefreshed) {
    ...
    const res = await retryOperation(async () => {
      return update({profileAgent: record.profileAgent});
    }, async () => {
      return get({
        id: record.profileAgent.id, includeSecrets: true
      });
    });
    const recordUpdated = res?.lastErrorObject?.updatedExisting;
    if(recordUpdated) {
      record = res.value;
    } else {
      // delete _id
...
async function _refreshProfileAgentUserDocZcaps({zcap, now, record} = {}) {
  let zcapRefreshed = false;
  const {zcapRefreshThreshold} = config.profile.profileAgent;
  ...
  console.log('profile agent user doc',
    JSON.stringify(profileAgentUserDoc.content, null, 2));

  // FIXME: new, replace above
  {
    const userDoc = await _readUserEdvDocument({profileAgentRecord: record});
    console.log('*****NEW profile agent user doc',
      JSON.stringify(userDoc.content, null, 2));
  ...
  const updatedProfileAgentUserDoc = klona(profileAgentUserDoc);
  ...
  for(const profileAgentUserDocZcapName in profileAgentUserDocZcaps) {
    if(profileAgentUserDocZcapName === 'profileCapabilityInvocationKey') {
      continue;
    }
    ...
    if((profileAgentUserDocZcapExpTime - now) <= zcapRefreshThreshold) {
      // refresh the zcap's expires property
      profileAgentUserDocZcap.expires = timestampToDateString(
        now + EDV_ZCAP_TTL);
      zcapRefreshed = true;
    }
  }
...
async function _readUserEdvDocument({profileAgentRecord}) {
  const invocationSigner = await getSigner({profileAgentRecord});
  const {zcaps} = profileAgentRecord.profileAgent;
  const edvClient = new EdvClient({capability: zcaps.userDocument, httpsAgent});
  const kmsClient = new KmsClient({httpsAgent});
