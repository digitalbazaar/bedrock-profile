...
export async function getAll({accountId, includeSecrets = false} = {}) {
  ...
  records = await _refreshProfileAgentZcaps({
...
async function _refreshProfileAgentZcaps({profileAgentRecords}) {
  ...
  return Promise.all(profileAgentRecords.map(record =>
    _refreshZcaps({record, now, zcapRefreshThreshold})));
}

async function _refreshZcaps({
  record, now, zcapRefreshThreshold
}) {
  ...
  for(const zcapName in zcaps) {
    ...
    if((expTime - now) <= zcapRefreshThreshold) {
      ...
      zcap.expires =
        new Date(now + EDV_ZCAP_TTL).toISOString().slice(0, -5) + 'Z';
      zcapRefreshed = true;
    }
  }
  if(zcapRefreshed) {
