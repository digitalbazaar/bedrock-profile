      ...
      if(zcapName !== 'profileCapabilityInvocationKey') {
        ...
        if(expTime - now <= zcapRefreshThreshold) {
          if(zcapName === 'userDocument') {
            ...
            const profileSigner = await _getProfileSigner({
              kmsClient, profileAgentRecord: record, zcaps
            });
            ...
            const edvConfig = await _getEdvConfig({edvClient, profileSigner});
            const docId = zcap.invocationTarget.split('/').pop();
            // get profile agent user doc
            const profileAgentUserDoc = await _getEdvDocument({
            ...
            for(const profileAgentUserDocZcapName in profileAgentUserDocZcaps) {
              if(
                ...
                if(
                  ...
                  console.log(`won't get here for now`);
            ...
            if(zcapRefreshed) {
              await edvClient.update({doc: updatedProfileAgentUserDoc});
            }
          }
...
async function _getEdvConfig({edvClient, profileSigner} = {}) {
...
async function _getProfileSigner({
  kmsClient, profileAgentRecord, zcaps
} = {}) {
  const profileCapabilityInvocationKeyZcap =
    zcaps.profileCapabilityInvocationKey;
  const {data} = profileAgentRecord.secrets.seed;
