...
export async function get({
  ...
  // default to excluding `secrets`
  const projection = {_id: 0, secrets: 0};
  // include secrets if requested or if reconciling
...
export async function update({
  ...
  // exclude `secrets` by default from returned record
  const projection = {_id: 0, secrets: 0};
  if(includeSecrets) {
    // do not exclude secrets per request
    delete projection.secrets;
  }
  ...
  // decrypt any secrets in the record if `secrets` are to be returned
  let record = result.value;
  if(includeSecrets) {
    // decrypt record secrets according to configuration
...
async function _reconcileProfileAgentRecords({records}) {
  ...
  const reconciled = await Promise.all(records.map(async record => {
    ...
    return _completeSecretsEncryption({record: profileAgentRecord});
  }));
...
async function _completeSecretsEncryption({record}) {
  ...
  // decrypt encrypted secrets now that they are encrypted in storage
...
async function _getProfileAgentRecords({accountId}) {
  ...
  return Promise.all(records.map(
    // decrypt record secrets according to configuration
    async record => decryptRecordSecrets({record})));
}
