      ...
      if(zcapName !== 'profileCapabilityInvocationKey') {
        ...
        if(expiresTimestamp - now <= zcapRefreshThreshold) {
          if(zcapName === 'userDocument') {
            ...
            // read from the EDV
            const url =
              new URL(zcaps.userDocument.invocationTarget);
            const rootEdvUrl = `${url.origin}/edvs`;
            const edvConfig = await EdvClient.findConfig({
              url: rootEdvUrl,
              controller: record.profileAgent.profile,
              referenceId: 'user',
              invocationSigner: profileInvocationSigner,
              httpsAgent
            });
            console.log(edvConfig.keyAgreementKey, 'edvConfig.keyAgreementKey');
            console.log(edvConfig.hmac, 'edvConfig.hmac');
