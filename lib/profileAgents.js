...
bedrock.events.on('bedrock-mongodb.ready', async () => {
  ...
  await promisify(database.createIndexes)([{
    ...
    collection: 'profile-profileAgent',
    fields: {'profileAgent.token': 1},
    options: {
      partialFilterExpression: {'profileAgent.token': {$exists: true}},
      unique: true,
      background: false
    }
  }]);
...
exports.create = async ({accountId, profileId, token} = {}) => {
  // FIXME: is accountId required or optional? The api description implies that
  // it may be, but the jsdoc does not have the expected brackets
  assert.string(accountId, 'accountId');
  // FIXME: is profileId required or optional? Tests in the suite are not
  // passing a profileId
...
exports.delegateCapabilityInvocationKey = async ({profileAgent, invoker}) => {
  const {
