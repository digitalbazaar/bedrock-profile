...
export async function getAll({accountId, includeSecrets = false} = {}) {
  ...
  const records = await _getProfileAgentRecords({query, projection});
...
async function _getProfileAgentRecords({query, projection}) {
  const collection = getCollection('profile-profileAgent');
...
async function _refreshProfileAgentZcaps({profileAgentRecords}) {
  ...
  const copy = JSON.parse(JSON.stringify(profileAgentRecords));
