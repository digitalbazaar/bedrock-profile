...
const {AsymmetricKey, Hmac, Kek, KeystoreAgent, KeyAgreementKey, KmsClient} =
...
api.createKeystore = async ({capabilityAgent, referenceId} = {}) => {
...
//        fixed in the future
api.updateKeystoreController = async ({id, controller}) => {
  const {httpsAgent} = brHttpsAgent;
  ...
  if(!updated) {
    throw new Error('Failed to update keystore with new controller');
  }
  return config;
};

api.getKeystore = async ({id} = {}) => {
  const {httpsAgent} = brHttpsAgent;
  return KmsClient.getKeystore({id, httpsAgent});
};

api.getKeystoreAgent = ({capabilityAgent, keystore} = {}) => {
  const {httpsAgent} = brHttpsAgent;
...
api.generateKey = async ({type, invocationSigner, kmsClient, kmsModule}) => {
  let Class;
