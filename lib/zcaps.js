...
exports.delegateCapability = async ({signer, edvClient, request} = {}) => {
  const {
    ...
    allowedAction, caveat, expires, type
  } = request;
  ...
  if(SUPPORTED_KEY_TYPES.includes(targetType)) {
    ...
    zcap.invocationTarget = target;
    let parentZcap;
    if(target) {
      const keystoreId = utils.parseKeystoreId(target);
    ...
    if(target) {
      ...
      zcap.invocationTarget = `${edvClient.id}/documents/${docId}`;
    ...
    if(!parentCapability) {
      ...
      const idx = zcap.invocationTarget.lastIndexOf('/');
      const docId = zcap.invocationTarget.substr(idx + 1);
    ...
    if(target) {
      ...
      zcap.invocationTarget = `${edvClient.id}/documents`;
    }
    if(!parentCapability) {
...
exports.delegate = async ({zcap, signer}) => {
  const {parentCapability} = zcap;

  // FIXME: How do we want to handle zcaps that dont contain expiration dates.
  if(!zcap.expires) {
    zcap.expires = new Date(new Date().getFullYear() + 100, 1).toISOString();
  }
  let suite;
  if(signer.type === 'Ed25519VerificationKey2018') {
