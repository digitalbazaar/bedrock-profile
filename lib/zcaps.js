...
exports.delegateCapability = async (
  ...
  if(expires) {
    const expiresPattern =
    new RegExp('^[1-9][0-9]{3}-(0[1-9]|1[0-2])-([0-2][0-9]|3[0-1])' +
    'T([0-1][0-9]|2[0-3]):([0-5][0-9]):(([0-5][0-9])|60)(\\.[0-9]+)?' +
    '(Z|((\\+|-)([0-1][0-9]|2[0-3]):([0-5][0-9])))?$');
    if(!expiresPattern.test(expires)) {
      throw new TypeError('"expires" must be an ISO date string.');
    }
