 ...
 * @param {string} [method='key'] - DID method to use for key id.
 * @param {string} [mode='live'] - Relevant for v1 type dids, test or live.
 * @returns {Promise<string>} Resolves with generated key id.
 */
exports.computeKeyId = async ({key, method = 'key', mode = 'live'} = {}) => {
  ...
  if(method === 'key') {
    ...
    throw new Error(`DID method not supported: '${method}'`);
  }
  return keyId;
};
