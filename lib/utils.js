...
export async function getProfileSigner({kmsClient, profileAgentRecord} = {}) {
  const record = JSON.parse(JSON.stringify(profileAgentRecord));
  const profileCapabilityInvocationKeyZcap =
    record.profileAgent.zcaps.profileCapabilityInvocationKey;
...
export async function removeSecretsFromRecords({records}) {
