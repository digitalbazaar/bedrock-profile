...
let EDV_ZCAP_TTL;
let PROFILE_ZCAP_INVOCATION_KEY_ZCAP_TTL;

bedrock.events.on('bedrock.start', async () => {
  ({
    edvZcapTtl: EDV_ZCAP_TTL,
    profileCapabilityInvocationKeyZcapTtl: PROFILE_ZCAP_INVOCATION_KEY_ZCAP_TTL
...
async function _delegateEdvsZcaps({
  key, keystore, edvs, controller, ttl = EDV_ZCAP_TTL
}) {
  const allZcaps = {};
...
async function _delegateEdvZcaps({
  key, keystore, edv, controller, ttl = EDV_ZCAP_TTL
}) {
...
async function _delegateEdvDocumentsZcap({
  key, edvId, controller, ttl = EDV_ZCAP_TTL
}) {
  return delegate({
