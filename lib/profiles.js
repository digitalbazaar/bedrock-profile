 ...
 * @param {object} options.profileAgentKmsMeterCapability - The meter
 *   capability to give to the KMS service when creating keystores associated
 *   with the new profile's root capability agent.
 * @param {string} [options.profileAgentKmsModule] - The KMS module to use to
 *   create the profile agent's keystore.
 * @param {object} options.profileKmsMeterCapability - The meter capability to
 *   give to the KMS service when creating keystores associated with the
 *   profile.
 * @param {string} [options.profileKmsModule] - The KMS module to use to create
 *   the profile's keystore.
...
module.exports.create = async ({
  accountId, didMethod,
  profileAgentKmsMeterCapability, profileAgentKmsModule,
  profileKmsMeterCapability, profileKmsModule,
  didOptions
} = {}) => {
  assert.string(accountId, 'accountId');
  assert.string(didMethod, 'didMethod');
  assert.object(
    profileAgentKmsMeterCapability, 'profileAgentKmsMeterCapability');
  assert.optionalString(profileAgentKmsModule, 'profileAgentKmsModule');
  assert.object(profileKmsMeterCapability, 'profileKmsMeterCapability');
  assert.optionalString(profileKmsModule, 'profileKmsModule');
  ...
  const {profileAgent, secrets} = await profileAgents.create({
    accountId,
    kmsMeterCapability: profileAgentKmsMeterCapability,
    kmsModule: profileAgentKmsModule
  });
  ...
  const profileKeystore = await kms.createKeystore({
    ...
    meterCapability: profileKmsMeterCapability,
    kmsModule: profileKmsModule
  });
