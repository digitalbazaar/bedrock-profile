...
module.exports.create = async ({
  accountId, didMethod, didOptions, privateKmsBaseUrl, publicKmsBaseUrl
} = {}) => {
  ...
  const profileKeystore = await kms.createKeystore({
    ...
    // FIXME: There was a bug in bedrock-kms that did not properly have a
    // compound index on controller and referenceId. When bedrock-kms with that
    // fix is applied
    // here, the keyStore being created here is in conflict with the keystore
    // being created in profileAgents.js#L108. If referenceId is uncommented
    // here there is a duplicate keystore error. This codepath is tested via
    // the test 20-profiles.js#L31.
    // referenceId: 'primary',
  });
