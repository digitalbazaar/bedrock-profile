...
export async function create({
  ...
  /* 5. Write the profile agent record to the database, including the meters
    ...
    step.** */
  const {edvMeter, kmsMeter} = await _insertProfileAgentRecord({
  ...
  await profileAgents.update({profileAgent: newProfileAgent});

  /* 8. Return the profile ID, meters, and EDV configs. */
  const edvConfigs = {}
