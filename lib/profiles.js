...
export async function create({
  ...
  profileAgentRecord.meta.meters = {edvMeter, kmsMeter};
...
/*
  ...
  // FIXME: remove
  const profileAgentRecord2 = await profileAgents.create({
    keystoreOptions: keystoreOptions.profileAgent, accountId,
    store: false
  })
  await profileAgents.insert({record: profileAgentRecord2});
  const {secrets: secrets2} = profileAgentRecord2;

  const {profileAgent: profileAgent2} = profileAgentRecord2;
  const {capabilityAgent: paZcapAgent} = await profileAgents.getAgents(
    {profileAgent: profileAgent2, secrets: secrets2});
  ...
  profileAgent2.profile = profileId2;

  await profileAgents.update({
    profileAgent: {
      ...profileAgent2,
      sequence: profileAgent2.sequence + 1,
      profile: profileId2,
      zcaps: {
  ...
  const [, , , ...meters] = await Promise.all([
    ...
    _updateMeterController({...edvOptions.profile, controller: profileId2}),
    profileMeters.add({meter: kmsMeter2}),
    profileMeters.add({meter: edvMeter2})
  ]);
  ...
  return {id: profileId2, meters, edvs: edvConfigs};
}
