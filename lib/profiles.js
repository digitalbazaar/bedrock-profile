...
exports.create = async ({accountId} = {}) => {
  ...
  // FIXME: if profileId needs to be a required param in the create API,
  // then a placeholder must be used here until the profile is updated
  // to a did:key later in this function
  const {profileAgent} = await profileAgents.create({
    accountId, profileId: 'TEMPORARY_PROFILE_ID'});
